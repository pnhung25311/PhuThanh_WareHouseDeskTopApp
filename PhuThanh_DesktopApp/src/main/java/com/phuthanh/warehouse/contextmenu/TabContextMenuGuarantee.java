...
import com.phuthanh.warehouse.screen.dialog.DialogCreateGuarantee;
...
public class TabContextMenuGuarantee {
    ...
    private   void onUpdate(String codeAID, Runnable callback) {
        try {
            FXMLLoader loader = new FXMLLoader(
                    TabContextMenuHandler.class.getResource("/fxml/dialogCreateGuarantee.fxml"));
            Parent root = loader.load();
            DialogCreateGuarantee controller = loader.getController();
            controller.setInitialData(callback, true, codeAID);

            Stage dialog = new Stage();
            dialog.setTitle("Cập nhật phiếu bảo hành");
    ...
    private   void onDelete(String codeAID, Runnable callback) {
        try {
            ...
            int row = dbCRUDHelper.delete("Guarantee", Arrays.asList("GuaranteeAID"),
                    Arrays.asList(codeAID));
            if (row > 0) {
                customDialogNotification.showDialog("Thành công", "Xóa phiếu bảo hành thành công",
                ...
                customDialogNotification.showDialog("Lỗi", "Xóa phiếu bảo hành thất bại",
