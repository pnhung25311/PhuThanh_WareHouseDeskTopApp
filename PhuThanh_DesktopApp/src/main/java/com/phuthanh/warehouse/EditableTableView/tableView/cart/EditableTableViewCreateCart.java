...
public class EditableTableViewCreateCart {
    ...
    private final int typeCart; // 1 là nhập hàng, 2 là xuất hàng, 3 là điều chuyển

    private final DbCRUDHelper dao = new DbCRUDHelper();

    public EditableTableViewCreateCart(int type) {
        this.typeCart = type;
        createTable();
    ...
    public void saveToDatabase() throws SQLException {
        ...
        for (CartFX c : data) {
            ...
            rows.add(Arrays.asList(
                    ...
                    safe(c.getInvoiceNumber().get()), safe(c.getRemark().get()), typeCart, now));
        }

        int[] rowsAffected = dao.insertBatch("Cart", columns, rows);
